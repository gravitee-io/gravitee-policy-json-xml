...
import static com.graviteesource.policy.json2xml.transformer.JSONTokener.DEFAULT_MAX_DEPTH;
import static com.graviteesource.policy.v3.json2xml.JsonToXmlTransformationPolicyV3.CONTENT_TYPE;
...
class JsonToXmlTransformationPolicyTest {
    ...
    void shouldInterruptWhenInvalidJsonOnRequest() throws IOException {
        final String invalidInput = loadResource("/com/graviteesource/policy/json2xml/invalid-input.json");
        when(request.onBody(onBodyCaptor.capture())).thenReturn(Completable.complete());
    ...
    void shouldInterruptWhenMaxNestedObjectOnRequest() throws IOException {
        final String invalidInput = loadResource("/com/graviteesource/policy/json2xml/invalid-embedded-object.json");
        when(request.onBody(onBodyCaptor.capture())).thenReturn(Completable.complete());
        when(request.headers()).thenReturn(HttpHeaders.create());
    ...
    void shouldInterruptWhenMaxNestedArrayOnRequest() throws IOException {
        final String invalidInput = loadResource("/com/graviteesource/policy/json2xml/invalid-embedded-array.json");
        when(request.onBody(onBodyCaptor.capture())).thenReturn(Completable.complete());
        when(request.headers()).thenReturn(HttpHeaders.create());
